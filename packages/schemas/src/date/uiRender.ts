...
export const makeElementPlainTextContentEditable = (element: HTMLElement) => {
  ...
  // Validate and format input as MM/DD/YYYY with a 4-digit year
  element.addEventListener('input', (e: Event) => {
    ...
    // Split the input text by slashes
    ...
    inputText = month || '';
    if (day) inputText += '/' + day;
    if (year) inputText += '/' + year;

    // Ensure the length doesn't exceed 10 characters (MM/DD/YYYY)
  ...
  element.addEventListener('keydown', (e: KeyboardEvent) => {
    ...
    // Prevent any non-numeric keys except for the slash
    if (!e.key.match(/^[0-9/]$/)) {
      e.preventDefault();
    }
  });
