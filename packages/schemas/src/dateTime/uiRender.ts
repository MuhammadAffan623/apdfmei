...
export const makeElementPlainTextContentEditable = (element: HTMLElement) => {
  ...
  // Handle input events for both date and time
  element.addEventListener('input', (e: Event) => {
    const target = e.target as HTMLElement;
    let inputText = target.innerText;

    // Split into date and time parts
    let [datePart, timePart, periodPart] = inputText.split(' ');

    // Process date part (MM/DD/YYYY)
    if (datePart && !timePart) {
      ...
      if (datePart.length === 2 || datePart.length === 5) {
        datePart += '/';
      }
    }

    // Process time part (HH:MM AM/PM)
    if (timePart) {
      timePart = timePart.replace(/[^0-9: APMapm]/g, '');
    ...
    // Combine date and time parts back into inputText
    inputText = datePart || '';
    if (timePart) {
      inputText += ' ' + timePart;
    }
  ...
  element.addEventListener('keydown', (e: KeyboardEvent) => {
    ...
    // Allow only numeric keys, slashes, colon, space, and AM/PM characters
