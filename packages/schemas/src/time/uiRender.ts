...
export const makeElementPlainTextContentEditable = (element: HTMLElement) => {
  ...
  element.addEventListener('input', (e: Event) => {
    ...
    let inputText = target.innerText;

    // Remove any non-numeric characters except for colon, space, A, P, M
    inputText = inputText.replace(/[^0-9: APMapm]/g, '');

    // Split the input into time parts and period (AM/PM)
    let [timePart, periodPart] = inputText.split(' ');

    // Handle time part (HH:MM)
    if (timePart) {
      let timeParts = timePart.split(':');
      let hour = timeParts[0] ? timeParts[0].replace(/[^0-9]/g, '') : '';
      let minute = timeParts[1] ? timeParts[1].replace(/[^0-9]/g, '') : '';

      // Automatically add colon after hour part
      if (hour.length === 2) {
        hour = hour + ':';
      }
      ...
      // Automatically add a space after the minute part if period is missing
      if (minute.length === 2 && !periodPart) {
        timePart += ' ';
      }
    }
    ...
    if (periodPart) {
      ...
      if (periodPart.startsWith('A')) {
        periodPart = 'AM';
      } else if (periodPart.startsWith('P')) {
        periodPart = 'PM';
      } else {
        periodPart = '';
      }
  ...
  element.addEventListener('keydown', (e: KeyboardEvent) => {
    ...
    // Prevent any non-numeric keys except for the colon, space, and AM/PM
    if (!e.key.match(/^[0-9: APMapm]$/)) {
      e.preventDefault();
    }
  });

  // Helper function to set the caret at the end of the content
