...
These can be loaded as plugins from the `apdf-mei-schemas` package.

You can also create your own schemas and load them similarly as plugins.
This page explains how to use schemas from `apdf-mei-schemas` and how to create your own.

## Using Schemas from apdf-mei-schemas

Here, we explain how to import image and QR code schemas from `apdf-mei-schemas`.

First, install `apdf-mei-schemas`.

```bash
npm install apdf-mei-schemas
```

Next, import the required schemas from `apdf-mei-schemas` to `apdf-mei-generator` and `apdf-mei-ui`.

The following code shows an example of importing QR code and image schemas from `apdf-mei-generator` and `apdf-mei-ui`.
...
In this `apdf-mei-ui` example, we're using the Designer, but you can load plugins in the Form and Viewer in the same way.
...
- **pdf**: Used in `apdf-mei-generator`, it includes code for rendering schemas into PDFs. The PDF rendering process is handled by [pdf-lib](https://pdf-lib.js.org/).
- **ui**: Used in `apdf-mei-ui`, it includes code for rendering schemas into the DOM. The ui has the following modes:
...
- **propPanel**: Used in `apdf-mei-ui`'s [Designer](/docs/getting-started#designer), it allows you to add custom property editing forms to the sidebar when a field is selected. You can fill it out using [form-render](https://xrender.fun/form-render)'s JSON format (widget extensions are also possible).
...
### Learning How to Create from apdf-mei-schemas' Code

If you're looking to create your own schema, it is recommended to refer to the existing code within `apdf-mei-schemas` while doing so.  
